...
std::string FrameDetect="modbus_multi";
...
DWORD WINAPI ComReadThreadFunc(LPVOID lpParam)
{
    ...
    if (Parity.compare("EVEN")==0) mParity=EVENPARITY;
      ...
      hComSerial = CreateFile( mPORT.c_str(), // TEXT("\\\\.\\COM3"),
       ...
       serialParams.Parity = mParity; //EVENPARITY;
        ...
        double Modbus_1_5Char;
        double Modbus_Frame_Delay;
        double Modbus_3_5Char;
        ...
        double dblBitsPerByte = 1 + serialParams.ByteSize + (serialParams.StopBits+1) + ( serialParams.Parity>0? 1 : 0 );
        double Serial_1_Char=dblBitsPerByte / (double)serialParams.BaudRate * 1000000.0f;
        if(serialParams.BaudRate>19200)
        {
            Modbus_1_5Char=750;
            Modbus_3_5Char=1750;
        }
        ...
        {
            Modbus_1_5Char=1.5f * Serial_1_Char;
            Modbus_3_5Char=3.5f * Serial_1_Char;
        }

        double ReadTimeOut=1.5f * Serial_1_Char;  //Default ReadTimeOut is 1.5 CHAR
        double ReadFrameDelay=std::stod(FrameDelay);
        double ReadFrameMulti=std::stod(FrameMulti);

        if (strcmp(FrameDetect.c_str(), "modbus_multi") == 0)ReadTimeOut=((ReadFrameMulti *  Modbus_1_5Char)+ReadFrameDelay)/1000;
        if (strcmp(FrameDetect.c_str(), "char_multi") == 0)ReadTimeOut=((ReadFrameMulti * Serial_1_Char)+ReadFrameDelay)/1000;
        if (strcmp(FrameDetect.c_str(), "char_delay") == 0)ReadTimeOut=ReadFrameDelay/1000;


         // Set timeouts
        COMMTIMEOUTS timeout = { 0 };
        //timeout.ReadIntervalTimeout = MAXDWORD; //MAXDWORD;
        timeout.ReadIntervalTimeout = (DWORD)(double)(ceil(ReadTimeOut)); //MAXDWORD;
...
if (!SetCommMask(hComSerial, EV_RXCHAR));
...
bool CheckForMoreData;
bool MoreDataTimeout;
double MoreDataTime;
while(hComSerial !=INVALID_HANDLE_VALUE)
{
   ...
   if (WaitCommEvent(hComSerial, &dwCommEvent, NULL))
   {
        ...
        if(!FrameVector.empty())
        {
            ...
            SetEvent(hCommFrameReadyEvent);
        }
        
    // OutputQueue.push(FrameVector);
   }
   else
   {
...
int CreateComThread()
{
    ...
    if (hCommReadThread == INVALID_HANDLE_VALUE || hCommFrameReadyEvent == INVALID_HANDLE_VALUE || hCommReadThread==NULL || hCommFrameReadyEvent==NULL)
    {
...
void print_extcap_config_comport()
{
    ...
    for (int i=1; i<256; i++)
        ...
        szPort[0] = TEXT('\0');
        sprintf(szPort, TEXT("\\\\.\\COM%u"), i);

        //Try to open the port
        HANDLE hCom=CreateFile(szPort, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, 0, 0);
...
{
    std::string  argString="";  
    argString+="arg {number=5}{call=--frame_detect}{display=Interframe Detection}{type=selector}\n";
    argString+="value {arg=5}{value=modbus_multi}{display=Multipler :1.5X Modbus Character}{default=true}\n";
    argString+="value {arg=5}{value=char_multi}{display=Multipler: 1X Character}{default=false}\n";
    argString+="value {arg=5}{value=char_delay}{display=Delay Only}{default=false}\n";
    argString+="arg {number=6}{call=--frame_multi}{display=Interframe Multipler}{type=double}{range=1,15}{default=2.0}\n";
    argString+="arg {number=7}{call=--frame_delay}{display=Interframe Delay(us)}{type=double}{range=0,15}{default=0.0}\n";
    argString+="arg {number=8}{call=--frame_correct}{display=Interframe Correction}{type=selector}\n";
    argString+="value {arg=8}{value=none}{display=None}{default=true}\n";
    argString+="value {arg=8}{value=modbus_crc}{display=Modbus CRC}{default=false}\n";
    printf("%s", argString.c_str());
}
...
{
    std::string  argString="";  
    argString+="arg {number=9}{call=--dlt}{display=Wireshark DLT}{type=selector}\n";
    argString+="value {arg=9}{value=250}{display=250:  RTAC Serial}{default=true}\n";
    argString+="value {arg=9}{value=147}{display=147:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=148}{display=148:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=149}{display=149:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=150}{display=150:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=151}{display=151:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=152}{display=152:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=153}{display=153:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=154}{display=154:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=155}{display=155:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=156}{display=156:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=157}{display=157:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=158}{display=158:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=159}{display=159:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=160}{display=160:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=161}{display=161:  User DLT}{default=false}\n";
    argString+="value {arg=9}{value=162}{display=162:  User DLT}{default=false}\n";
    printf("%s", argString.c_str());
}
...
void print_extcap_dlt(std::string sInterface)
{
    ...
    //printf("dlt {number=%u}{name=ModbusRTU Adapter}{display=ModbusRTU Adapter}\n",  DLT_MODBUS_RTU);
    printf("%s", argString.c_str());
}
    ...
    {   
        ...
        do {
            ...
            case 8:
                ...
                break;   
            case 9:
                sArgOpt="--frame_detect";
                pArgOptValue=&FrameDetect;
                break;   
            case 10:
                sArgOpt="--frame_multi";
...
DWORD WINAPI ControlInThreadFunc(LPVOID lpParam)
{
    ...
    while (hPipe == INVALID_HANDLE_VALUE)
    {
        ...
        /* if an error occured at handle creation */
        if (!WaitNamedPipe(ControlInPipeName.c_str(), 20000)) {
            printf("Could not open pipe: waited for 20sec!\n"
                "If this message was issued before the 20sec finished,\n"
                "then the pipe doesn't exist!\n");
            return 0;
