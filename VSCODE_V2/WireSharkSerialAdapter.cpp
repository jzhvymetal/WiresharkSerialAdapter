...
DWORD WINAPI ComReadThreadFunc(LPVOID lpParam)
{
    ...
    if (Parity.compare("EVEN")==0) mParity=EVENPARITY;
      ...
      hComSerial = CreateFile( mPORT.c_str(), // TEXT("\\\\.\\COM3"),
        ...
        if(serialParams.BaudRate>19200 && strcmp(FrameDetect.c_str(), "modbus_multi") == 0)
        {
            Serial_1_Char=500;
        }
        ...
        if (strcmp(FrameDetect.c_str(), "char_delay") == 0)ReadTimeOut=ReadFrameDelay;
        ...
        timeout.ReadIntervalTimeout = MAXDWORD; //Use delay wait
...
while(hComSerial !=INVALID_HANDLE_VALUE)
{
   ...
   { 
        do
        {
            ...
            {
                EV_Timeout=EV_Timeout+1000;  //Timeout is fixed to 1ms=1000us
            }
...
void print_extcap_config_interframe()
{
    ...
    argString+="value {arg=5}{value=modbus_multi}{display=Multipler :1X Modbus Character}{default=true}\n";
    ...
    argString+="arg {number=6}{call=--frame_multi}{display=Interframe Multipler}{type=double}{range=1,15}{default=3.0}\n";
